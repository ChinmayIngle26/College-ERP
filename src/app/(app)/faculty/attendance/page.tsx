...
export default function FacultyAttendancePage() {
    ...
    const groupedReportRecords = useMemo(() => {
        ...
        // Convert the nested object structure into sorted arrays
        return Object.values(groupsByDate)
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) // Sort dates descending
            .map(dateGroup => ({
                ...dateGroup,
                lectures: Object.values(dateGroup.lectures) // Convert lectures object to array
            }));
    }, [reportRecords]);
    ...
    const handleDownloadReport = () => {
        if (reportRecords.length === 0 || currentStudents.length === 0) {
            toast({ title: "No Data to Export", description: "Please fetch records before downloading a report.", variant: "destructive" });
            return;
        }

        // 1. Get unique lectures and sort them
        const uniqueLectures = Array.from(new Map(reportRecords.map(r => [`${r.date}-${r.lectureName}`, r])).values())
            .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

        // 2. Create a map of attendance for quick lookup
        const attendanceMap = new Map<string, string>(); // Key: 'studentId-date-lectureName', Value: status
        reportRecords.forEach(r => {
            attendanceMap.set(`${r.studentId}-${r.date}-${r.lectureName}`, r.status === 'present' ? 'P' : 'A');
        });

        // 3. Transform data for CSV
        const dataForCsv = currentStudents.sort((a,b) => (a.studentIdNumber || '').localeCompare(b.studentIdNumber || '')).map((student, index) => {
            ...
            uniqueLectures.forEach(lecture => {
                const lectureKey = `${format(new Date(lecture.date), 'yyyy-MM-dd')} - ${lecture.lectureName}`;
                const status = attendanceMap.get(`${student.userId}-${lecture.date}-${lecture.lectureName}`) || 'A';
                if (status === 'P') {
                    presentCount++;
                }
                studentRow[lectureKey] = status;
            });
            
            const attendancePercentage = uniqueLectures.length > 0 ? ((presentCount / uniqueLectures.length) * 100).toFixed(2) : "0.00";
            studentRow['% Attendance'] = attendancePercentage;

            // Reorder the percentage column to be after the name
            const { '% Attendance': percentage, ...rest } = studentRow;
            return {
                'Sr. No': studentRow['Sr. No'],
                'Roll No': studentRow['Roll No'],
                'Name of Student': studentRow['Name of Student'],
                                    ...
                                    <Table className="min-w-full">
                                        <TableHeader>
                                            <TableRow className="bg-muted/10">
                                                ...
                                                <TableHead className="w-[150px] px-3 py-2 text-center border">Overall %</TableHead>
                                        ...
                                        <TableBody>
                                            {filteredStudentsToDisplay.map((student, index) => (
                                                <TableRow key={student.userId}>
                                                    ...
                                                    <TableCell className="px-3 py-2 text-center border">N/A</TableCell>
